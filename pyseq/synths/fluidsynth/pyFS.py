...
import pathlib
lib = str(pathlib.Path(__file__).parent / "libfluidsynth-3.dll")
...
fluid_sfont_iteration_next = cfunc('fluid_sfont_iteration_start', c_void_p,
                                     ('sfont', c_void_p, 1))
...
class Synth:
    ...
    def sfpreset_name(self, sfid, bank, prenum):
        ...
        if fluid_synth_get_sfont_by_id is not None:
            ...
            return None
    def sfpreset_name_by_chan(self, chan):
    ...
    def tuning_dump(self, bank, prog, pitch):
        return fluid_synth_tuning_dump(self.synth, bank, prog, None, 0, pitch)
